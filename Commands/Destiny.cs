                ...
                using (var client = new HttpClient())
                {
                    ...
                    if (item != null)
                        for (var i = 0; i < item.Response.Count; i++)
                        {
                            ...
                            var memResponse = client.GetAsync("https://www.bungie.net/platform/Destiny2/" + memType +
                                                              "/Profile/" + memId + "/?components=100").Result;
                            ...
                            if ((int) memItem.Response.profile.data.userInfo.crossSaveOverride !=
                                (int) memItem.Response.profile.data.userInfo.membershipType &&
                                ((int) memItem.Response.profile.data.userInfo.crossSaveOverride != 0 ||
                                 (int) memItem.Response.profile.data.userInfo.membershipType != (int) Platform))
                                continue;
